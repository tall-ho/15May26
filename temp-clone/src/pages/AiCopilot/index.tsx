    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[${THEME.gold}] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b7a159] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg"><Icons.BrainCircuit size={22} className="text-[#b7a159]"/> SMART LAW GUIDE</h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5">Law Assistant Guide</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.Scale size={18} className="text-[#b7a159]"/> 1. Contract Analysis
            </h4>
            <p className="text-[12px] mb-3">AI สามารถช่วยคุณวิเคราะห์เอกสารทางกฎหมาย, ตรวจสอบสัญญา และระบุจุดเสี่ยง:</p>
            <ul className="list-none pl-0 space-y-3">
                <li className="flex items-start gap-2 bg-[#f8f9fa] p-3 rounded-xl border border-[#eaeaec]">
                  <Icons.FileText size={16} className="shrink-0 text-[#4d87a8] mt-0.5"/> 
                  <div><strong className="text-[#4d87a8]">Clause Review:</strong> ตรวจสอบข้อความในสัญญาเทียบกับมาตรฐานบริษัท</div>
                </li>
                <li className="flex items-start gap-2 bg-[#f8f9fa] p-3 rounded-xl border border-[#eaeaec]">
                  <Icons.ShieldCheck size={16} className="shrink-0 text-[#657f4d] mt-0.5"/> 
                  <div><strong className="text-[#657f4d]">Legal Compliance:</strong> ตรวจสอบความสอดคล้องกับกฎหมาย PDPA หรือ แรงงาน</div>
                </li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.Gavel size={18} className="text-[#d96245]"/> 2. Counseling Support
            </h4>
            <p className="text-[12px] mb-3">ให้ AI เป็นผู้ช่วยในการเตรียมข้อมูลเบื้องต้นสำหรับงานที่ปรึกษา:</p>
            <ul className="list-disc pl-5 mt-2 space-y-2 text-[12px]">
                <li><strong className="text-[#d96245]">Draft Summaries:</strong> สร้างสรุปใจความสำคัญของกฎหมายใหม่</li>
                <li><strong className="text-[#212c46]">Precedent Search:</strong> ช่วยระบุแนวทางจากคำพิพากษาหรือกรณีตัวอย่าง</li>
                <li><strong className="text-[#657f4d]">Legal Drafting:</strong> ช่วยร่างจดหมายบอกกล่าว หรือ ข้อความทางกฎหมายเบื้องต้น</li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            ...
            <p className="text-[12px] bg-[#3f809e]/10 p-3 rounded-xl border border-[#3f809e]/30 text-[#212c46]">เลือก <b>Prompt Recommendations</b> ที่เราเตรียมไว้ให้ หรือพิมพ์คำสั่ง เช่น "สรุปขั้นตอนการเลิกจ้างตามกฎหมายแรงงาน" เพื่อรับข้อมูลที่แม่นยำและรวดเร็ว</p>
          </section>
        </div>
...
export default function AiCopilot() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, role: 'ai', text: 'สวัสดีค่ะ! ฉันคือ In-house Counsel ผู้ช่วยส่วนตัวของคุณในระบบ SMART LAW. วันนี้มีข้อกฎหมายหรือเอกสารส่วนไหนที่คุณต้องการให้ฉันช่วยวิเคราะห์ไหมคะ?', timestamp: '08:00 AM' }
  ]);
  ...
  const handleSendMessage = (text: string) => {
    ...
    // Mock AI reply
    setTimeout(() => {
      let aiReply = "กำลังวิเคราะห์ข้อมูลทางกฎหมาย... ขออภัย ข้อมูลจำลองยังไม่เชื่อมต่อกับฐานข้อมูลจริง แต่คุณสามารถปรับแต่งการตั้งค่าการตอบกลับได้ในภายหลัง.";
      
      if (text.includes('Liability')) {
        aiReply = "ในการวิเคราะห์สัญญาจ้างบริการ (Service Agreement) จุดเสี่ยงของ Liability มักอยู่ที่:\n1. การจำกัดความรับผิด (Limitation of Liability) ว่าครอบคลุมถึงความเสียหายทางอ้อมหรือไม่\n2. ขอบเขตการชดใช้ค่าเสียหาย (Indemnification)\n3. เหตุสุดวิสัย (Force Majeure) ที่อาจไม่ครอบคลุมสถานการณ์ปัจจุบัน";
      } else if (text.includes('กฎหมายแรงงาน')) {
        aiReply = "ตามกฎหมายแรงงานปัจจุบัน:\n1. การเลิกจ้างพนักงานที่ทำผิดวินัยร้ายแรง (ตาม ม.119) นายจ้างไม่ต้องจ่ายค่าชดเชย\n2. การลาออก ปกติควรแจ้งล่วงหน้าอย่างน้อย 1 งวดการจ่ายค่าจ้าง\n3. หากเลิกจ้างโดยไม่มีความผิด ต้องจ่ายค่าชดเชยตามอายุงาน และค่าบอกกล่าวล่วงหน้า (ถ้าไม่ได้แจ้งล่วงหน้า)";
      } else if (text.includes('PDPA')) {
        aiReply = "การเก็บข้อมูลพนักงานภายใต้ PDPA:\n1. ต้องแจ้งวัตถุประสงค์ (Privacy Notice)\n2. เก็บข้อมูลเท่าที่จำเป็น (Data Minimization)\n3. จัดให้มีมาตรการรักษาความปลอดภัยของข้อมูลที่เหมาะสม";
      }
  ...
  return (
    <div className="flex flex-1 w-full flex-col pb-5 animate-fadeIn bg-transparent space-y-4">
          ...
          <div className="flex items-center gap-5">
              ...
              <div>
                  <h3 className="font-black text-[#212c46] uppercase tracking-tighter leading-none flex items-center gap-2" style={{ fontSize: '24px' }}>
                      IN-HOUSE <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#b58c4f] to-[#8e9141]">COUNSEL</span>
                  ...
                  <p className="text-[11px] font-bold text-[#b58c4f] uppercase tracking-[0.2em] mt-0.5 opacity-90 leading-none">
                      INTELLIGENT COMPLIANCE & LEGAL ASSISTANT
                  </p>
              </div>
          </div>
          ...
          <div className="flex items-center gap-4">
              <div className="bg-white/50 p-1.5 rounded-xl border border-white/60 shadow-inner flex flex-wrap items-center gap-1">
                  <button onClick={() => setActiveMode('chat')} className={`px-4 py-2 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeMode === 'chat' ? 'bg-[#212c46] text-white shadow-md' : 'text-[#7a8b95] hover:text-[#b58c4f]'}`}>
                    <Icons.MessageSquare size={16} /> Counsel Chat
                  </button>
      ...
      <div className="max-w-[1532px] mx-auto px-4 sm:px-8 w-full mt-2 flex-1 pb-6 min-h-0 flex flex-col">
        <div className="w-full flex-1 flex flex-col gap-6 lg:flex-row h-full min-h-0">
            ...
            <div className="w-full lg:w-80 flex flex-col gap-5 shrink-0 animate-fadeIn h-full">
                ...
                <div className="bg-white/90 p-4 rounded-3xl shadow-lg border border-[#eaeaec] flex flex-col items-center justify-center text-center">
                    ...
                    <h4 className="text-[13px] font-black text-[#212c46] uppercase tracking-widest font-mono">Counsel Bot</h4>
                ...
                <div className="flex-1 bg-white rounded-3xl shadow-lg border border-[#eaeaec] overflow-hidden flex flex-col animate-fadeIn relative">
                    ...
                    <div className="h-14 border-b border-[#eaeaec] bg-[#f8f9fa] flex items-center justify-between px-5 shrink-0 z-10">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-black text-[#212c46] uppercase text-[12px] tracking-widest">Counsel Conversation</span>
                        </div>
